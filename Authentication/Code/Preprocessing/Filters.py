...
class Filter():
    ...
    @staticmethod
    def high_pass_filter(data: np.ndarray, order: int, crit_freq: float, filter_type: str):
        b, a = signal.butter(order, crit_freq, filter_type, fs=250)
        high_pas_filtered_data = signal.filtfilt(b, a, data)
        return high_pas_filtered_data

    @staticmethod
    def filter_ecg(data: np.ndarray):
        ica_mean, mixing_matrix, sources = ICA.construct_ic_sources(data, 8)
        # ICA.plot_ica_sources(sources)
        ...
        for index, source in enumerate(sources.T):
            ...
            peaks, _ = find_peaks(energy, height=threshold, distance=150)
