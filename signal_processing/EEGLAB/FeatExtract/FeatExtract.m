...
% figure(3)
% s= spectrogram(epochs',hanning(L/3),L/6,2^nextpow2(L/3),Fs,'MinThreshold',-20,'yaxis');
% spectrogram(epochs',hanning(L/3),L/6,2^nextpow2(L/3),Fs,'MinThreshold',-20,'yaxis');
[S,F,T] = stft(epochs',Fs,'Window',hanning(L/3),'OverlapLength',L/6,'FFTLength',2^nextpow2(L/3));
...
% pwelch(epochs',hanning(L/3), L/6,nfftWelch,Fs,'psd');
% figure(5)
power= pwelch(epochs,hanning(L/3),L/6,nfftWelch, Fs,'power');  %[p,f]= pwelch(x,window,noverlap,nfft,fs) 
% pwelch(epochs,hanning(L/3),L/6, nfftWelch,Fs,'power'); 
