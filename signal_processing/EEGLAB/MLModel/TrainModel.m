function TrainModel(dataset, path, nFiles)
    ...
    [Fs, labels] = Label_extract(summarypath, EpochLengthSec, nFiles); %get labels of where there are seizures
    labels = labels'; %epochs in rows
    temp = [];
    for k = 1 : size(labels, 2) %loop through columns of labels
        temp = [temp; labels(:,k)]; %make labels 1 long column vector where every row is an epoch
    end
